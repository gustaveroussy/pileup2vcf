...
class MPileup(object):
    ...
    #L'idée cette fois est de parser le fichier mpileup, définir pour chaque ligne un objet variant, auquel on applique les filtres globaux.
    #Si il passe on le conserve sinon on l'écrit dans le fichier TrashWhileReading
    ...
    def parseAndFilter(self, printDepthProfile, depthProfileKeepInMemory, depthProfileBinDynamically,fh):
        ...
        * Returns: self.arrayPileup
        '''
        cur = Variant("", 0, "", 0, "", "", "", "", "", "")
        printTrashWhileReading(cur,fh,True)
        avancement = []
        ...
        for line in self.file:
            ...
            if cur.isVariant() is True:
                curPassFilter = cur.getGlobalFilterState()
                if(curPassFilter is "True"):
                    #print("NOT FILTER")
                    self.arrayPileup.append(cur)
                else:
                    #print("FILTER")
                    cur.call()
                    printTrashWhileReading(cur,fh,False)
                    del cur
